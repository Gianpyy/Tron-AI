...
class Player:
    def __init__(self, x, y, b, c):
        ...
        self.boosts = 3
        self.rect = pygame.Rect(self.x - 1, self.y - 1, 5, 5)  # player rect object

    def __draw__(self):
        """
        method for drawing player
        """
        self.rect = pygame.Rect(self.x - 1, self.y - 1, 5, 5)  # redefines rect
    ...
    def __move__(self):
        ...
        """
        # insert player position scaled by (width/matrix_size) -> 12 then resize x and y on new resize values
        # insert 1 or 2 into the matrix depending on the player's (change logic not depending on colours?)
        if self.colour == P1_COLOUR:
            map[int(self.y/12)][int(self.x/12)] = 1
        if self.colour == P2_COLOUR:
            map[int(self.y/12)][int(self.x/12)] = 2
...
pygame.display.set_caption("Tron-AI")  # sets window title
...
p1 = Player(200, 200, (2, 0), P1_COLOUR)  # creates player
p2 = Player(300,300, (-2, 0), P2_COLOUR)
objects.append(p1)
